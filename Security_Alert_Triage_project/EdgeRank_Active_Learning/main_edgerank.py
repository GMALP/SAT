...
from algorithm import main_process_er_solo
...
def prob_rank(prob, data_week, name, k):
    ...
    # 6. Count positive number n based on Equation 5
    print('6. Count positive number n based on Equation 5.')
    num_p_ = len(data_rank[data_rank['predict_probability'] >= 0.5])
    k_ = 2 * num_p_
    if not os.path.exists(save_path + 'results_sp/sp_ranking_res/'):
        os.makedirs(save_path + 'results_sp/sp_ranking_res/')
...
def prob_rank_usp(prob, data_week, name, k, index_):
    """
    :param: prob
    :param: data_week
    :param: name
    ...
    # Statistics p > 0.5 number
    print('6. Count positive number n based on Equation 5.')
    k_ = 2 * len(data_rank[data_rank['predict_probability'] >= 0.5])
    ''''''
    if index_ == 'fpr_fnr_p':
        if not os.path.exists(save_path + 'results_usp/alert/ranking_results/fpr_fnr_p/'):
            os.makedirs(save_path + 'results_usp/alert/ranking_results/fpr_fnr_p/')
        data_rank.to_excel(save_path + 'results_usp/alert/ranking_results/fpr_fnr_p/' + name + '_' + str(k)
                           + '_ranking_res.xlsx', index=False)
    else:
        if not os.path.exists(save_path + 'results_sp/sp_ranking_res/'):
            os.makedirs(save_path + 'results_sp/sp_ranking_res/')
        data_rank.to_excel(save_path + 'results_sp/sp_ranking_res/' + name + '_' + str(k) + '_ranking_res.xlsx',
...
def down_data_direction():
    ...
    excel_write = pd.ExcelWriter('./alert_data.xlsx')
...
def main_er_solo():
    ...
    print('Active Learning process begin ...')
    iteration_list = ['Week1', 'Week2', 'Week3', 'Week4', 'Week5', 'Week6', 'Week7', 'Week8']
    ...
    for iteration_name in iteration_list:
        ...
        # ranking algorithm
        edge_ranking_result, ranking_fpr_fnr_result = main_process_er_solo.edge_rank(data_iteration, iteration_name,
                                                                                     fall_acc, high_acc, low_acc)
        ...
        for k in sample_k_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                with open(save_path + 'models/Iteration' + last_iteration_name[4] + '_model_' + str(k) + '.pickle', 'rb') as f:
                    rfc = pickle.load(f)
                ...
                if k_compute == 'on':
                    ...
                    print('k set is %d, and compute_k is %d. ' % (k, compute_k))
                    if compute_k > k:
                        compute_k = k
                else:
                    compute_k = k
                ...
                if not os.path.exists(save_path + 'results_sp/rank/fpr_fnr_results/'):
                    os.makedirs(save_path + 'results_sp/rank/fpr_fnr_results/')
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'results_sp/rank/fpr_fnr_results/Iteration' + iteration_name[4] +
                ...
                if not os.path.exists(save_path + 'results_sp/rank/rer_mer_results/'):
                    os.makedirs(save_path + 'results_sp/rank/rer_mer_results/')
                rer_mer_res.to_excel(save_path + 'results_sp/rank/rer_mer_results/Iteration' + iteration_name[4] + '_' +
                ...
                if not os.path.exists(save_path + 'results_sp/class/fpr_fnr_results/'):
                    os.makedirs(save_path + 'results_sp/class/fpr_fnr_results/')
                sp_class_fpr_fnr_res.to_excel(save_path + 'results_sp/class/fpr_fnr_results/Iteration' +
                                              iteration_name[4] +
                                              '_' + str(k) + '_sp_ranking_fpr_and_fnr.xlsx', index=False)

                '''usp_rank_add_sp_res test'''
                x_usp_test, y_ = feature_extra(edge_ranking_result, dimension_data)
                prob_usp_test = rfc.predict_proba(x_usp_test)
                # sp ranking compute fpr and fnr
                data_usp_add_p, _ = prob_rank_usp(prob_usp_test, edge_ranking_result, iteration_name, k, 'fpr_fnr_p')
                # data_usp_add_p.sort_values(by=['predict_probability'], ascending=False)  # , inplace=True
                # sorted(data_usp_add_p['predict_probability'].tolist(), reverse=True)
                ranking_fpr_fnr_result['predict_probability'] = data_usp_add_p['predict_probability']

                if not os.path.exists(save_path + 'results_usp/alert/ranking_results/usp_rank_add_sp_res/'):
                    os.makedirs(save_path + 'results_usp/alert/ranking_results/usp_rank_add_sp_res/')
                ranking_fpr_fnr_result.to_excel(
                    save_path + 'results_usp/alert/ranking_results/usp_rank_add_sp_res/Iteration' + iteration_name[4] +
                    '_ranking_res.xlsx', index=False)
                ...
                train_data_before = pd.read_excel(save_path + 'train_data/Iteration' + last_iteration_name[4] + '_'
                ...
                train_data = pd.concat([train_data_before, select_data_usp, select_data_sp_top], ignore_index=True)

                # save train_data to file
                print('save train alert data length %d. ' % (len(train_data)))
                if not os.path.exists(save_path + 'train_data/'):
                    os.makedirs(save_path + 'train_data/')
                train_data.to_excel(save_path + 'train_data/Iteration' + iteration_name[4] + '_' + str(k) + '_sample_data'
                                    + '.xlsx', index=False)
                ...
                with open(save_path + 'models/Iteration' + iteration_name[4] + '_model_' + str(k) + '.pickle', 'wb') as f:
                    pickle.dump(rfc, f)
                gc.collect()
            else:
                print('the first iteration begin...')
                # ranking algorithm
                # edge_ranking_result = main_process.edge_rank(data_iteration, iteration_name, fall_acc, high_acc,
                # low_acc)

                # select top k data to train
                sample_data = edge_ranking_result.head(k)

                # save select_data to file
                print('save first sample alert data length %d. ' % (len(sample_data)))
                if not os.path.exists(save_path + 'train_data/'):
                    os.makedirs(save_path + 'train_data/')
                sample_data.to_excel(save_path + 'train_data/Iteration' + iteration_name[4] + '_' + str(k) +
