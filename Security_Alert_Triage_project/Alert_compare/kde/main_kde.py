...
def sp_prob_rank(prob, data_week, name, k):
    ...
    k_ = 2 * num_p_
    # data_rank.to_excel(save_path + '/kde/results_sp/sp_ranking_res/' + name + '_' + str(k) + '_ranking_res.xlsx',
    # index=False)
...
def get_suit_bandwidth(x_train):
    ...
    grid = GridSearchCV(
        ...
        param_grid={'bandwidth': 10 ** np.linspace(-1, 1, 10)},
        cv=LeaveOneOut()
    )
...
def kde_rank(data_week_data, iteration_name, dim_data):
    ...
    # kde
    # bandwidth = get_suit_bandwidth(x_feature)
    # bandwidth = np.std(x_feature)
    # print('parameter bandwidth set is %f ' % bandwidth)
    model = KernelDensity(kernel='gaussian', bandwidth=100)
    model.fit(x_feature)
    ...
    # data_rank.to_excel(save_path + 'kde/results_usp/ranking_results/Iteration' + iteration_name[4] +
    # '_kde_ranking.xlsx',
    # index=False)
...
def main_kde(data_all_label, dimension_data, iteration_list, sample_k_list):
    ...
    print('edge: running kde algorithm...')
    for iteration_name in iteration_list:
        ...
        usp_ranking_fpr_fnr_result.to_excel(
            save_path + 'kde/results_usp/fpr_fnr_results/Iteration' + iteration_name[4] + '_ranking_fpr_and_fnr.xlsx',
            index=False)

        # alert ranking RE and MER evaluate
        ...
        for k in sample_k_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'kde/results_sp/rank/fpr_fnr_results/Iteration' + iteration_name[4] + '_'
                ...
                rer_mer_res.to_excel(save_path + 'kde/results_sp/rank/rer_mer_results/Iteration' + iteration_name[4] + '_' + str(k) +
                ...
                sp_class_fpr_fnr_res.to_excel(save_path + 'kde/results_sp/class/fpr_fnr_results/Iteration' + iteration_name[4] + '_'
                                              + str(k) + '_sp_ranking_fpr_and_fnr.xlsx', index=False)
                ...
                train_data_before = pd.read_excel(save_path + 'kde/train_data/Iteration' + last_iteration_name[4] +
                ...
                train_data.to_excel(save_path + 'kde/train_data/Iteration' + iteration_name[4] + '_' + str(k) +
