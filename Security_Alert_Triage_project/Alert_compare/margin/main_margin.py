...
"""
@ Author:
@ file name: main_margin.py
@ Date: 2022/01/26 16:00
@ describe: margin algorithm test
@ tools: pycharm
"""
...
def train_process_(data_we, dim_data, k):
    ...
    first_se_train_data = data_we.sample(n=k, random_state=10)
    if not os.path.exists(save_path + 'margin/train_data/'):
        os.makedirs(save_path + 'margin/train_data/')
    first_se_train_data.to_excel(save_path + 'margin/train_data/Iteration1_' + str(k) + '_sample_data.xlsx', index=False)
    # print('selected train data length %d. ' % len(first_se_train_data))
    ...
    # save model
    if not os.path.exists(save_path + 'margin/models/'):
        os.makedirs(save_path + 'margin/models/')
    with open(save_path + 'margin/models/Iteration1_model_' + str(k) + '.pickle', 'wb') as f:
        pickle.dump(rfc_first, f)

    return rfc_first


def margin_rank(data_, iteration_name, dim_data, rfc):
    """margin_rank"""
    ...
    # data_rank.to_excel('./RF_ratio_' + str(k) + '_Prediction_probability.xlsx', index=False)

    # Descending order
    data_rank.sort_values(by=['p1'], ascending=False, inplace=True)  # ascending：False  True
    # data_rank.to_excel(save_path + 'margin/results_usp/ranking_results/Iteration' + iteration_name[4] +
    # '_rank_entropy.xlsx',
    # index=False)
...
def main_margin(data_all_label, dimension_data, iteration_list, sample_k_list):
    """margin algorithm test entry"""
    print('edge: running margin algorithm...')
        ...
        for iteration_name in iteration_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                if iteration_name != 'Week2':
                    with open(save_path + 'margin/models/Iteration' + last_iteration_name[4] + '_model_' + str(k) + '.pickle', 'rb') as f:
                        rfc = pickle.load(f)
                else:
                    with open(save_path + 'margin/models/Iteration' + last_iteration_name[4] + '_model_' + str(k) + '.pickle',
                              'rb') as f:
                        rfc = pickle.load(f)
                ...
                if not os.path.exists(save_path + 'margin/results_sp/rank/fpr_fnr_results/'):
                    os.makedirs(save_path + 'margin/results_sp/rank/fpr_fnr_results/')
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'margin/results_sp/rank/fpr_fnr_results/Iteration' + iteration_name
                ...
                if not os.path.exists(save_path + 'margin/results_sp/rank/rer_mer_results/'):
                    os.makedirs(save_path + 'margin/results_sp/rank/rer_mer_results/')
                rer_mer_res.to_excel(save_path + 'margin/results_sp/rank/rer_mer_results/Iteration' + iteration_name[4] + '_' +
                ...
                if not os.path.exists(save_path + 'margin/results_sp/class/fpr_fnr_results/'):
                    os.makedirs(save_path + 'margin/results_sp/class/fpr_fnr_results/')
                sp_class_fpr_fnr_res.to_excel(save_path + 'margin/results_sp/class/fpr_fnr_results/Iteration' + iteration_name[4]
                                              + '_' + str(k) + '_sp_ranking_fpr_and_fnr.xlsx', index=False)

                # ranking algorithm
                entropy_ranking_result = margin_rank(data_week_label, iteration_name, dimension_data, rfc)
                ...
                if not os.path.exists(save_path + 'margin/results_usp/fpr_fnr_results/'):
                    os.makedirs(save_path + 'margin/results_usp/fpr_fnr_results/')
                usp_ranking_fpr_fnr_result.to_excel(save_path + 'margin/results_usp/fpr_fnr_results/Iteration' +
                                                    iteration_name[4] +
                                                    '_ranking_fpr_and_fnr.xlsx', index=False)

                # alert RE and MER evaluate
                usp_rer_mer_res = ranking_rer_mer_compute.rer_mer_compute(entropy_ranking_result)
                if not os.path.exists(save_path + 'margin/results_usp/rer_mer_results/'):
                    os.makedirs(save_path + 'margin/results_usp/rer_mer_results/')
                usp_rer_mer_res.to_excel(save_path + 'margin/results_usp/rer_mer_results/Iteration' + iteration_name[4] +
                ...
                if iteration_name != 'Week2':
                    train_data_before = pd.read_excel(save_path + 'margin/train_data/Iteration' + last_iteration_name[4]
                                                      + '_' + str(k) + '_sample_data.xlsx')
                else:
                    train_data_before = pd.read_excel(save_path + 'margin/train_data/Iteration' + last_iteration_name[4]
                                                      + '_' + str(k) + '_sample_data.xlsx')
                ...
                if not os.path.exists(save_path + 'margin/train_data/'):
                    os.makedirs(save_path + 'margin/train_data/')
                train_data.to_excel(save_path + 'margin/train_data/Iteration' + iteration_name[4] + '_' + str(k) +
                ...
                if not os.path.exists(save_path + 'margin/models/'):
                    os.makedirs(save_path + 'margin/models/')
                with open(save_path + 'margin/models/Iteration' + iteration_name[4] + '_model_' + str(k) + '.pickle', 'wb') as f:
                    pickle.dump(rfc, f)
                gc.collect()
            else:
                # print('the first iteration begin...')
                # ranking algorithm
                with open(save_path + 'margin/models/Iteration' + iteration_name[4] + '_model_' + str(k) + '.pickle', 'rb') as f:
                    rfc = pickle.load(f)
                margin_ranking_result = margin_rank(data_week_label, iteration_name, dimension_data, rfc)
                usp_ranking_fpr_fnr_result = ranking_fpr_fnr_compute.ranking_fpr_fnr_compute(margin_ranking_result,
                                                                                             'alert')
                # compute optimize fpr and fnr
                real_idx, opt_fpr_fnr = process.get_cross(usp_ranking_fpr_fnr_result, iteration_name, 'results_usp')
                usp_ranking_fpr_fnr_result['opt_index'] = real_idx
                ...
                # Alert RE and MER evaluate
                usp_rer_mer_res = ranking_rer_mer_compute.rer_mer_compute(margin_ranking_result)
                if not os.path.exists(save_path + 'margin/results_usp/rer_mer_results/'):
                    os.makedirs(save_path + 'margin/results_usp/rer_mer_results/')
                usp_rer_mer_res.to_excel(
                    save_path + 'margin/results_usp/rer_mer_results/Iteration' + iteration_name[4] + '_rer_mer.xlsx',
                    index=False)
                ...
                if not os.path.exists(save_path + 'margin/results_sp/rank/fpr_fnr_results/'):
                    os.makedirs(save_path + 'margin/results_sp/rank/fpr_fnr_results/')
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'margin/results_sp/rank/fpr_fnr_results/Iteration' + iteration_name[4]
                ...
                if not os.path.exists(save_path + 'margin/results_sp/rank/rer_mer_results/'):
                    os.makedirs(save_path + 'margin/results_sp/rank/rer_mer_results/')
                rer_mer_res.to_excel(save_path + 'margin/results_sp/rank/rer_mer_results/Iteration' + iteration_name[4] + '_' + str(k)
                                    + '_rer_mer.xlsx', index=False)
    ...
    print('margin algorithm process done.')
