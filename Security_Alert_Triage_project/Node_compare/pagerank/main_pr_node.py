...
def fixed_size(feature):
    ...
    for j, key in enumerate(host_ip):
        ...
        feature_out.append([feature_mid, label])  # , label
...
def prob_rank(prob, data_week, name, k):
    ...
    k_ = 2 * num_p_
    # data_rank.to_excel(save_path + 'pagerank/results_sp/sp_ranking_res/' + name + '_' + str(k) + '_ranking_res.xlsx',
    # index=False)
...
def main_pr(data_all, data_all_label, dimension_data, iteration_list, sample_k_list, fall_acc, high_acc, low_acc):
    ...
    for iteration_name in iteration_list:
        ...
        # ranking algorithm
        edge_ranking_result = main_process.pagerank(data_iteration, iteration_name, fall_acc, high_acc, low_acc)

        for k in sample_k_list:
            print('Initial sample k %d alerts data...' % k)

            if last_iteration_name != 'Week0':
                print('Using %s model predict %s alerts data...' % (last_iteration_name, iteration_name))
                # feature extraction
                ...
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'pagerank/results_sp/rank/fpr_fnr_results/Iteration' +
                                                   iteration_name[4] + '_' + str(k) + '_ranking_fpr_and_fnr.xlsx',
                                                   index=False)

                #  host RE and MER evaluate
                rer_mer_res = ranking_rer_mer_compute.rer_mer_compute(data_sp_rank)
                if not os.path.exists(save_path + 'agerank/results_sp/rank/rer_mer_results/'):
                    os.makedirs(save_path + 'pagerank/results_sp/rank/rer_mer_results/')
                rer_mer_res.to_excel(save_path + 'pagerank/results_sp/rank/rer_mer_results/Iteration' + iteration_name[4]
                                     + '_' + str(k) +
                ...
                train_data_before = pd.read_excel(save_path + 'pagerank/train_data/Iteration' + last_iteration_name[4]
                ...
                with open(save_path + 'pagerank/models/Iteration' + iteration_name[4] + '_model_' + str(k) + '.pickle',
                          'wb') as f:
                    pickle.dump(rfc, f)
                gc.collect()
            else:
                ...
                # ranking algorithm
                # edge_ranking_result = main_process.pagerank(data_iteration, iteration_name, fall_acc, high_acc,
                # low_acc)
