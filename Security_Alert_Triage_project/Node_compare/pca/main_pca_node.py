...
def get_chinese_feature(logs):
    ...
    for i in range(len(logs['源IP'])):
        ...
        host_label = logs['label'].tolist()[i]
...
def sp_prob_rank(prob, data_week, name, k):
    ...
    k_ = 2 * num_p_
    # data_rank.to_excel(save_path + 'pca/results_sp/sp_ranking_res/' + name + '_' + str(k) + '_ranking_res.xlsx',
    # index=False)
...
def pca_rank(data_week_data, iteration_name, dim_data):
    """kde ranking"""
    ...
    # pca
    pca = PCA()  # n_components=min(2, len())  n_components='mle'
    pca.fit(x_feature)
    y_pred = pca.predict(x_feature)
    prob = pca.predict_proba(x_feature, method='unify')  # method='unify', 'linear'

    abnormal_index, abnormal_prob, predict_s = [], [], []
    for i in range(len(y_pred)):
        abnormal_index.append(i)
        abnormal_prob.append(prob[i][1])
        predict_s.append(y_pred[i])

    dic = {'abnormal_index': abnormal_index, 'abnormal_prob': abnormal_prob, 'predict_s': predict_s}
    ...
    for ii in range(len(data_rank)):
        prob_save.append(df_prob['abnormal_prob'].to_list()[ii])
        predict_save.append(df_prob['predict_s'].to_list()[ii])

    data_rank['预测概率'] = prob_save
    data_rank['predict_label'] = predict_save
    # data_rank.to_excel(save_path + 'pca/results_usp/ranking_results/Iteration' + iteration[4] +
    # '_pca_ranking_res.xlsx', index=False)
...
def main_pca(data_all_label, dimension_data, iteration_list, sample_k_list):
    """pca algorithm test entry"""
    ...
    for iteration_name in iteration_list:
        ...
        # ranking algorithm
        usp_ranking_result = pca_rank(data_iteration_label, iteration_name, dimension_data)
        # node ranking fpr and fnr evaluate
        usp_ranking_fpr_fnr_result = ranking_fpr_fnr_compute.ranking_fpr_fnr_compute(usp_ranking_result, 'host')
        ...
        if not os.path.exists(save_path + 'pca/results_usp/fpr_fnr_results/'):
            os.makedirs(save_path + 'pca/results_usp/fpr_fnr_results/')
        usp_ranking_fpr_fnr_result.to_excel(
            save_path + 'pca/results_usp/fpr_fnr_results/Iteration' + iteration_name[4] +
            '_ranking_fpr_and_fnr.xlsx', index=False)

        # node ranking RE and MER evaluate
        pca_rer_mer_res = ranking_rer_mer_compute.rer_mer_compute(usp_ranking_result)
        if not os.path.exists(save_path + 'pca/results_usp/rer_mer_results/'):
            os.makedirs(save_path + 'pca/results_usp/rer_mer_results/')
        pca_rer_mer_res.to_excel(
            save_path + 'pca/results_usp/rer_mer_results/Iteration' + iteration_name[4] + '_rer_mer.xlsx', index=False)
        '''
        for k in sample_k_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                with open(save_path + 'pca/models/Iteration' + last_iteration_name[4] + '_model_' + str(k) + '.pickle', 'rb') as f:
                    rfc = pickle.load(f)
                ...
                if not os.path.exists(save_path + 'pca/results_sp/rank/fpr_fnr_results/'):
                    os.makedirs(save_path + 'pca/results_sp/rank/fpr_fnr_results/')
                sp_ranking_fpr_fnr_result.to_excel(save_path + 'pca/results_sp/rank/fpr_fnr_results/Iteration' + iteration_name[4] +
                ...
                if not os.path.exists(save_path + 'pca/results_sp/rank/rer_mer_results/'):
                    os.makedirs(save_path + 'pca/results_sp/rank/rer_mer_results/')
                rer_mer_res.to_excel(save_path + 'pca/results_sp/rank/rer_mer_results/Iteration' + iteration_name[4] + '_' + str(k) +
                                     '_rer_mer.xlsx', index=False)

                # alert classification performance
                sp_class_fpr_fnr_res = ranking_fpr_fnr_compute.classification_fpr_fnr_compute(data_sp_rank, 'host')
                # compute optimize fpr and fnr
                ...
                if not os.path.exists(save_path + 'pca/results_sp/class/fpr_fnr_results/'):
                    os.makedirs(save_path + 'pca/results_sp/class/fpr_fnr_results/')
                sp_class_fpr_fnr_res.to_excel(save_path + 'pca/results_sp/class/fpr_fnr_results/Iteration' +
                                              iteration_name[4] + '_'
                ...
                train_data_before = pd.read_excel(save_path + 'pca/train_data/Iteration' + last_iteration_name[4] +
                ...
                if not os.path.exists(save_path + 'pca/train_data/'):
                    os.makedirs(save_path + 'pca/train_data/')
                train_data.to_excel(save_path + 'pca/train_data/Iteration' + iteration_name[4] + '_' + str(k) +
                ...
                # save model
                if not os.path.exists(save_path + 'pca/models/'):
                    os.makedirs(save_path + 'pca/models/')
                with open(save_path + 'pca/models/Iteration' + iteration_name[4] + '_model_' + str(k) + '.pickle', 'wb') as f:
                    pickle.dump(rfc, f)
                gc.collect()
            else:
                ...
                if not os.path.exists(save_path + 'pca/train_data/'):
                    os.makedirs(save_path + 'pca/train_data/')
                select_data.to_excel(save_path + 'pca/train_data/Iteration' + iteration_name[4] + '_' + str(k)
                                     + '_sample_data' + '.xlsx', index=False)
                ...
                gc.collect()
        '''
    print('pca algorithm process done.')
