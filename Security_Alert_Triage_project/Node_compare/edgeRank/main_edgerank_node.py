...
def prob_rank(prob, data_iteration, name, k):
    ...
    k_ = 2 * num_p_
    '''
    if not os.path.exists('./results_sp/sp_ranking_res/'):
        os.makedirs('./results_sp/sp_ranking_res/')
...
def main_er(data_all, data_all_label, dimension_data, iteration_list, sample_k_list, fall_acc, high_acc, low_acc):
    ...
    print('running edgerank(proposed) algorithm...')
    ...
    for iteration_name in iteration_list:
        ...
        for k in sample_k_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                train_data_before = pd.read_excel(save_path + 'edgeRank/train_data/Iteration' + last_iteration_name[4]
    ...
    print('running Edgerank(proposed) algorithm done.')
