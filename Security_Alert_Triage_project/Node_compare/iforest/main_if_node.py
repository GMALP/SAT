...
def sp_prob_rank(prob, data_week, name, k):
    ...
    for i in range(len(data_rank)):
        if len(list(df_prob.columns)) == 2:
            try:
                ...
                print('prob_save error = ', i, len(df_prob[1].tolist()))
        else:
    ...
    k_ = 2 * num_p_
    # data_rank.to_excel(save_path + 'iforest/results_sp/sp_ranking_res/' + name + '_' + str(k) + '_ranking_res.xlsx',
    # index=False)
...
def if_rank(data_week_data, iteration_name, dim_data):
    ...
    # data_rank.to_excel(save_path + 'iforest/results_usp/ranking_results/' + iteration_name + '_if_ranking_res.xlsx',
    # index=False)
    ...
    for iteration_name in iteration_list:
        ...
        for k in sample_k_list:
            ...
            if last_iteration_name != 'Week0':
                ...
                rer_mer_res.to_excel(save_path + 'iforest/results_sp/rank/rer_mer_results/Iteration' +
                ...
                sp_class_fpr_fnr_res.to_excel(save_path + 'iforest/results_sp/class/fpr_fnr_results/Iteration' + iteration_name[4] + '_'
                ...
                train_data_before = pd.read_excel(save_path + 'iforest/train_data/Iteration' + last_iteration_name[4] + '_' + str(k)
                                                  + '_sample_data.xlsx')
                train_data = pd.concat([train_data_before, select_data], ignore_index=True)
                ...
                train_data.to_excel(save_path + 'iforest/train_data/Iteration' + iteration_name[4] + '_' + str(k) +
                ...
                if not os.path.exists(save_path + 'iforest/train_data/'):
                    os.makedirs(save_path + 'iforest/train_data/')
                select_data.to_excel(save_path + 'iforest/train_data/Iteration' + iteration_name[4] + '_' + str(k) + '_sample_data' +
                                     '.xlsx', index=False)
